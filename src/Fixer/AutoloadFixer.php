...
readonly class AutoloadFixer implements FixerInterface
{
    ...
    public function fix(Baseline $baseline, ToolResolver $resolver): FixerResult
    {
        ...
        if ($composer === null) {
            return new FixerResult(label: $this->getLabel(), skipped: true, message: 'skipped (composer not found)');
        }

        $autoloadFile = $resolver->getProjectRoot() . '/vendor/composer/autoload_classmap.php';
        if (file_exists($autoloadFile)) {
            return new FixerResult(label: $this->getLabel(), skipped: true, message: 'already optimized');
        }
        ...
        return new FixerResult(label: $this->getLabel(), message: $result['errorOutput']);
    }
}
