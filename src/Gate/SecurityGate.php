...
readonly class SecurityGate implements GateInterface
{
    ...
    public function run(Baseline $baseline, ToolResolver $resolver): GateResult
    {
        ...
        if ($rules['hardcoded_secrets'] ?? true) {
            $f = $this->checkHardcodedSecrets($root, $paths);
            $findingsByRule['hardcoded_secrets'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }

        if ($rules['sql_injection'] ?? true) {
            $f = $this->checkSqlInjection($root, $paths);
            $findingsByRule['sql_injection'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }

        if ($rules['command_injection'] ?? true) {
            $f = $this->checkCommandInjection($root, $paths);
            $findingsByRule['command_injection'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }

        if ($rules['csrf_protection'] ?? true) {
            $f = $this->checkCsrf($root);
            $findingsByRule['csrf_protection'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }

        if ($rules['path_traversal'] ?? true) {
            $f = $this->checkPathTraversal($root, $paths);
            $findingsByRule['path_traversal'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }

        if ($rules['insecure_deserialization'] ?? true) {
            $f = $this->checkInsecureDeserialization($root, $paths);
            $findingsByRule['insecure_deserialization'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }

        if ($rules['ssrf'] ?? true) {
            $f = $this->checkSsrf($root, $paths);
            $findingsByRule['ssrf'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }

        if ($rules['tls_verification'] ?? true) {
            $f = $this->checkTlsVerification($root, $paths);
            $findingsByRule['tls_verification'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }

        if ($rules['insecure_rng'] ?? true) {
            $f = $this->checkInsecureRng($root, $paths);
            $findingsByRule['insecure_rng'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }

        if ($rules['gitignore_sensitive'] ?? true) {
            $f = $this->checkGitignore($root);
            $findingsByRule['gitignore_sensitive'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }

        if ($rules['package_freshness'] ?? true) {
            $f = $this->checkPackageFreshness($root);
            $findingsByRule['package_freshness'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }

        if ($rules['weak_cryptography'] ?? true) {
            $f = $this->checkWeakCryptography($root, $paths);
            $findingsByRule['weak_cryptography'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }

        if ($rules['cors_config'] ?? true) {
            $f = $this->checkCorsConfig($root);
            $findingsByRule['cors_config'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }

        $composerAudit = $this->runComposerAudit($baseline, $resolver);
        $allFindings = array_merge($allFindings, $composerAudit['findings']);
        $findingsByRule['composer_audit'] = $composerAudit['findings'];
        $criticalAdvisoryCount = $composerAudit['critical'];

        if ($rules['npm_audit'] ?? true) {
            $f = $this->checkNpmAudit($root);
            $findingsByRule['npm_audit'] = $f;
            $allFindings = array_merge($allFindings, $f);
        }
     ...
     */
    private function checkHardcodedSecrets(string $root, array $paths): array
    {
        $findings = [];
        $files = $this->scanPhpFiles($paths, self::EXCLUDE_PATHS_WITH_TESTS);
     ...
     * @return array<int, string>
     */
    private function checkSqlInjection(string $root, array $paths): array
    {
        $findings = [];

        foreach ($this->scanLines($paths, self::EXCLUDE_PATHS) as [$pathname, $i, $line]) {
            foreach (self::SQL_INJECTION_PATTERNS as $pattern) {
                if (preg_match($pattern, $line)) {
                    $findings[] = $this->fmt($root, $pathname, $i, $line, 'SQL injection risk');
                    break;
                }
            }
        }
     ...
     * @return array<int, string>
     */
    private function checkCommandInjection(string $root, array $paths): array
    {
        $findings = [];
        ...
        foreach ($this->scanLines($paths, self::EXCLUDE_PATHS) as [$pathname, $i, $line]) {
            ...
            $findings[] = $this->fmt($root, $pathname, $i, $line, 'Command injection risk');
        }
     ...
     * @return array<int, string>
     */
    private function checkCsrf(string $root): array
    {
     ...
     * @return array<int, string>
     */
    private function checkPathTraversal(string $root, array $paths): array
    {
        $findings = [];
        $userInput = self::USER_INPUT_SOURCES;

        foreach ($this->scanLines($paths, self::EXCLUDE_PATHS_WITH_TESTS) as [$pathname, $i, $line]) {
            ...
            if (preg_match('/Storage::(?:disk\([^)]*\)->)?(?:get|put|putFileAs|move|copy|delete|download|readStream|writeStream)\s*\(\s*\$(?:'.$userInput.')\b/', $line)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'Storage:: with user-controlled path');

                continue;
            }

            if (preg_match('/\b(file_get_contents|fopen|readfile|file_put_contents)\s*\(\s*\$(?:'.$userInput.')\b/', $line, $m)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, $m[1].'() with user-controlled path');
            ...
            if (preg_match('/(?:file_get_contents|fopen|readfile|file_put_contents|Storage::|include|require)\b[^;]*[\'"]\w+[\'"]\s*\.\s*\$(?:'.$userInput.')\b/', $line)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'File operation with concatenated user input');
            }
        }
     ...
     * @return array<int, string>
     */
    private function checkInsecureDeserialization(string $root, array $paths): array
    {
        $findings = [];
        ...
        foreach ($this->scanLines($paths, self::EXCLUDE_PATHS) as [$pathname, $i, $line]) {
            if (preg_match('/\bunserialize\s*\(\s*\$(?!this)[a-zA-Z_]/', $line)) {
                ...
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'unserialize() with dynamic input');
            }

            if (preg_match('/unserialize\s*\(\s*base64_decode/', $line) && ! $this->isInsideString($line, 'unserialize')) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'unserialize(base64_decode(...)) — exploit chain');
            }
        }
     ...
     * @return array<int, string>
     */
    private function checkSsrf(string $root, array $paths): array
    {
        $findings = [];
        $userInput = self::USER_INPUT_SOURCES;
        ...
        foreach ($this->scanLines($paths, self::EXCLUDE_PATHS_WITH_TESTS) as [$pathname, $i, $line]) {
            if (preg_match('/\bHttp::(?:get|post|put|patch|delete|head|send)\s*\(\s*\$(?:'.$userInput.')\b/', $line)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'Http:: called with user-controlled URL');
            ...
            if (preg_match('/->(?:request|get|post|put|patch|delete|head|send)\s*\(\s*(?:["\'][A-Z]+["\']\s*,\s*)?\$(?:'.$userInput.')\b/', $line)) {
                ...
                if ($content !== false && (str_contains($content, 'GuzzleHttp') || preg_match('/Guzzle|GuzzleHttp|HttpClient|Client/', $line))) {
                    $findings[] = $this->fmt($root, $pathname, $i, $line, 'HTTP client called with user-controlled URL');
            ...
            if (preg_match('/\b(?:file_get_contents|fopen|get_headers|readfile)\s*\(\s*\$(?:'.$userInput.')\b/', $line)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'Remote call with user-controlled URL');

                continue;
            }

            if (preg_match('/curl_setopt\s*\([^,]+,\s*CURLOPT_URL\s*,\s*\$(?:'.$userInput.')\b/', $line)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'curl_setopt(CURLOPT_URL) with user-controlled URL');
            }
        }
     ...
     * @return array<int, string>
     */
    private function checkTlsVerification(string $root, array $paths): array
    {
        $findings = [];

        foreach ($this->scanLines($paths, self::EXCLUDE_PATHS_WITH_TESTS) as [$pathname, $i, $line]) {
            if (preg_match('/->withoutVerifying\s*\(/', $line) || preg_match('/\bHttp::withoutVerifying\s*\(/', $line)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'withoutVerifying() disables TLS');

                continue;
            }

            if (preg_match('/["\']verify["\']\s*=>\s*false/', $line)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, '\'verify\' => false disables TLS');

                continue;
            }

            if (preg_match('/CURLOPT_SSL_VERIFY(?:PEER|HOST)\s*,\s*(?:false|0)\b/i', $line)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'CURLOPT_SSL_VERIFYPEER/HOST disabled');

                continue;
            }

            if (preg_match('/["\']verify_peer(?:_name)?["\']\s*=>\s*false/', $line)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'verify_peer disabled in stream context');

                continue;
            }

            if (preg_match('/["\']allow_self_signed["\']\s*=>\s*true/', $line)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'allow_self_signed enabled');
            }
        }
     ...
     * @return array<int, string>
     */
    private function checkInsecureRng(string $root, array $paths): array
    {
        $findings = [];
        ...
        foreach ($this->scanLines($paths, self::EXCLUDE_PATHS_WITH_TESTS) as [$pathname, $i, $line]) {
            if (preg_match('/\b(rand|mt_rand|uniqid)\s*\(/', $line, $m)
                ...
                $findings[] = $this->fmt($root, $pathname, $i, $line, $m[1].'() used for a secret/token (use random_bytes)');
            }
        }
     ...
     * @return array<int, string>
     */
    private function checkGitignore(string $root): array
    {
        ...
        foreach (self::GITIGNORE_REQUIRED_PATTERNS as $pattern) {
            ...
            if (! $this->sensitiveFileExists($root, $pattern)) {
                continue;
            }
        ...
        if (file_exists($envPath)) {
            exec('git -C '.escapeshellarg($root).' ls-files --error-unmatch .env 2>/dev/null', $out, $code);
            if ($code === 0) {
    ...
    private function sensitiveFileExists(string $root, string $pattern): bool
    {
        if (str_starts_with($pattern, '*')) {
            return glob($root.'/'.$pattern) !== [];
        }

        return file_exists($root.'/'.$pattern);
    }
     ...
     * @return array<int, string>
     */
    private function checkPackageFreshness(string $root): array
    {
     ...
     * @return array<int, string>
     */
    private function checkWeakCryptography(string $root, array $paths): array
    {
        $findings = [];

        foreach ($this->scanLines($paths, self::EXCLUDE_PATHS_WITH_TESTS) as [$pathname, $i, $line]) {
            if (preg_match('/\bmcrypt_[a-z_]+\s*\(/', $line)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'mcrypt_* is deprecated and broken');

                continue;
            }

            if (preg_match('/openssl_(?:encrypt|decrypt)\s*\([^)]*[\'"][^"\']*-ecb[\'"]/i', $line)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'ECB cipher mode is insecure (use CBC/GCM)');

                continue;
            }

            if (preg_match('/[\'"](?:des|3des|rc4|rc2)(?:-[a-z0-9]+)?[\'"]/i', $line) && preg_match('/openssl_(?:encrypt|decrypt)/', $line)) {
                $findings[] = $this->fmt($root, $pathname, $i, $line, 'DES/3DES/RC4 cipher is broken');
            ...
            if (preg_match('/\b(md5|sha1)\s*\(/i', $line, $m)
                ...
                $findings[] = $this->fmt($root, $pathname, $i, $line, "weak hash {$m[1]}() in security context");
            }
        }
     ...
     * @return array<int, string>
     */
    private function checkCorsConfig(string $root): array
    {
        $configPath = $root.'/config/cors.php';
     ...
     */
    private function runComposerAudit(Baseline $baseline, ToolResolver $resolver): array
    {
     ...
     * @return array<int, string>
     */
    private function checkNpmAudit(string $root): array
    {
        if (! file_exists($root.'/package.json')) {
            return [];
        }

        $hasLock = file_exists($root.'/package-lock.json')
            || file_exists($root.'/yarn.lock')
            || file_exists($root.'/pnpm-lock.yaml');

        if (! $hasLock) {
            return [];
        }

        exec('which npm 2>/dev/null', $out, $code);
        if ($code !== 0) {
            return [];
        }

        $process = new Process(['npm', 'audit', '--json'], $root, timeout: 120);
        $process->run();
